...
(function () {
  ...
  /* The seed menu, with three deliberate differences: a renamed item, a course
     ...
     Plus one addition that is a fixture rather than a difference. The
     expandable row this file checks the rebinding of used to be a real menu
     item — the crêpe and its seven toppings — and on 2026-08-06 the menu was
     resynced to the printed sheets, which do not have it. Nothing else on the
     site carries options today.

     So the row is manufactured here instead of hoping one exists. The
     behaviour under test belongs to render.js and script.js, not to the
     dessert list, and tying it to whatever the owner happens to be selling
     meant a real regression could go unnoticed the moment an item was
     retired — which is exactly what nearly happened. OPTS_AT names the item it
     is hung on, and is asserted below so a reshuffle of the seed data fails
     loudly here rather than quietly turning this check off. */
  var OPTS_AT = { page: "food", ci: 0, ii: 1 };
  function rows() {
    ...
    Object.keys(pages).forEach(function (page) {
      pages[page].forEach(function (c, ci) {
        ...
        (c.items || []).forEach(function (it, ii) {
          iid++;
          var fixture = page === OPTS_AT.page && ci === OPTS_AT.ci && ii === OPTS_AT.ii;
          var row = {
            ...
            options_dom_id: fixture ? "replayOpts" : (it.optionsId || null),
            sort_order: ii,
            menu_item_pours: (it.pours || []).map(function (p, pi) {
              return { id: "p" + iid + "_" + pi, label: p.label, price: p.price, sort_order: pi };
            }),
            menu_item_options: fixture
              ? [{ id: "o" + iid + "_0", name: "A topping", price: "1.00", sort_order: 0 }]
              : (it.options || []).map(function (o, oi) {
                  return { id: "o" + iid + "_" + oi, name: o.name, price: o.price, sort_order: oi };
                })
          };
          if (fixture) window.__optsFixtureOn = row.name;
          items.push(row);
        });
      });
    });
...
(function () {
  /* Counting .mi[data-opts] nodes would prove nothing — the question is how
     ...
     The row is the fixture the probe hangs on OPTS_AT — see the note there for
     why it is manufactured rather than taken from the menu.

     Today it is the *zero* case that can actually happen — the row is
  ...
  function report() {
    ...
    document.title = JSON.stringify({
      ...
      optsFixtureOn: window.__optsFixtureOn || null,
...
/* The fixture has to have been hung on something, or the check below is
   testing the absence of a row rather than the binding of one. */
if (!result.optsFixtureOn) {
  fail("the options fixture was never attached on " + PAGE,
       "OPTS_AT in the probe points at an item that no longer exists — " +
       "move it to one that does, or this check is silently off");
} else pass(`the options fixture is on "${result.optsFixtureOn}"`);

if (result.optionRow === null) {
  fail("the expandable row did not render on " + PAGE,
       "the probe supplies one; renderItem should have given it a .mi__toggle");
